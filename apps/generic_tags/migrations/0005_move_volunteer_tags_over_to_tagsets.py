...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        volunteer_ts = orm.TagSet.objects.get_or_create(name="Volunteer")[0]
        tsm_ct = ContentType.objects.get_for_model(orm.TagSetMembership)
        v_ct = ContentType.objects.get(app_label="volunteers",model="Volunteer")

        for vt in orm["taggit.taggeditem"].objects.filter(content_type=v_ct):
            person = orm["volunteers.volunteer"].objects.get(id=vt.object_id).person
            print person

            v_tsm = orm.TagSetMembership.objects.get_or_create(person=person, tagset=volunteer_ts)[0]

            # update the taggeditem
            vt.content_type_id=tsm_ct.id
            vt.object_id = v_tsm.id
            vt.save()
    ...
    models = {
        ...
        'volunteers.volunteer': {
            ...
            'status': ('django.db.models.fields.CharField', [], {'default': "('active', 'Active')", 'max_length': '50'})
        }
    }

    complete_apps = ['volunteers', 'generic_tags']
