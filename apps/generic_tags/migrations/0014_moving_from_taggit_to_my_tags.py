...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # create tags and taggeditems
        for t in orm.TaggedTagSetMembership.objects.all():
            new_tag = orm["generic_tags.tag"].objects.get_or_create(name=t.tag.name, tagset=t.content_object.tagset)[0]
            orm["generic_tags.taggeditem"].objects.get_or_create(tag=new_tag, person=t.content_object.person)

        # delete old tags
        orm["taggit.tag"].objects.all().delete()

        # delete ttsms
        orm.TaggedTagSetMembership.objects.all().delete()

        # delete tsms
        orm.TagSetMembership.objects.all().delete()
