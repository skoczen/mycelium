...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for gm in orm.GroupMembership.objects.all():
            group = orm.Group.objects.filter(name__iexact=gm.tag.name)
            if group.count() == 1:
                group = group[0]
            elif group.count() == 0:
                group = orm.Group.objects.create(name=gm.tag.name)
            else:
                for g in group.all():
                    print g.name
                raise Exception, "Two groups with the same name exist: %s %s" % ([g for g in group.all()], gm.tag.name)
