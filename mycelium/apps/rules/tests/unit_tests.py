...
class TestRuleModelFunctions(QiUnitTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.left_side = LeftSide.objects_by_account(self.request).get(display_name="have a General tag that")
...
class TestPopulateRuleComponents(QiUnitTestMixin, RuleTestAbstractions, GroupTestAbstractions, TestCase):
    ...
    def test_general_tags(self):
        left_side = LeftSide.objects_by_account(self.request).get(display_name="have a General tag that")
        self.assertEqual(left_side.query_string_partial, "taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='General',name")
        self.assertEqualQuerySets(left_side.operators,  self._tag_operators )
        self.assertEqualQuerySets(left_side.right_side_types,  self._text_right_side_types)
    ...
    def test_donor_tags(self):
        ...
        self.assertEqual(left_side.query_string_partial, "taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='Donor',name")
        self.assertEqualQuerySets(left_side.operators,  self._tag_operators )
        self.assertEqualQuerySets(left_side.right_side_types,  self._text_right_side_types)
        self.assertEqual(left_side.add_closing_paren, True)
    
    def test_volunteer_tags(self):
        left_side = LeftSide.objects_by_account(self.request).get(display_name="have a Volunteer tag that")
        self.assertEqual(left_side.query_string_partial, "taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='Volunteer',name")
        self.assertEqualQuerySets(left_side.operators,  self._tag_operators )
        self.assertEqualQuerySets(left_side.right_side_types,  self._text_right_side_types)
        self.assertEqual(left_side.add_closing_paren, True)
    ...
    def test_custom_tag_1(self):
        # make a new tagset
        TagSet.objects_by_account(self.request).get_or_create(name="new test tagset")
        populate_rule_components_for_an_account(self.account)
        left_side = LeftSide.objects_by_account(self.request).get(display_name="have a new test tagset tag that")
        self.assertEqual(left_side.query_string_partial, "taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='new test tagset',name")
        self.assertEqualQuerySets(left_side.operators,  self._tag_operators )
        self.assertEqualQuerySets(left_side.right_side_types, self._text_right_side_types)
        self.assertEqual(left_side.add_closing_paren, True)
    ...
    #     left_side = LeftSide.objects_by_account(self.request).get(display_name="total donations in the last 12 months")
    ...
    #     left_side = LeftSide.objects_by_account(self.request).get(display_name="total volunteer hours in the last 12 months")
    ...
    def test_operator_display_name_and_query_string_model_for_is_exactly(self):
        o = Operator.objects_by_account(self.request).get(display_name="is exactly")
    ...
    def test_operator_display_name_and_query_string_model_for_is_not_exactly(self):
        o = Operator.objects_by_account(self.request).get(display_name="is not exactly")
        self.assertEqual(o.query_string_partial, "__iexact=")
        self.assertEqual(o.use_filter, False)

    def test_operator_display_name_and_query_string_model_for_contains(self):
        o = Operator.objects_by_account(self.request).get(display_name="contains")
    ...
    def test_operator_ordering(self):
        list_of_names = [o.display_name for o in Operator.objects_by_account(self.request).all()]
        target_list_of_names = [
            "contains",
            "does not contain",
            "is",
    ...
    def test_populate_cleans_up_unused_rules(self):
        # create a custom tag
        from generic_tags.models import TagSet
        ts1 = TagSet.objects_by_account(self.request).get_or_create(name="new test tagset")[0]
        ...
        # make sure it's gone
        self.assertEqual(LeftSide.objects_by_account(self.request).filter(display_name="have a new test tagset tag that").count(), 0)
...
class TestQuerySetGeneration(TestCase, RuleTestAbstractions, GroupTestAbstractions, QiUnitTestMixin, DestructiveDatabaseTestCase):
    ...
    def test_create_new_group_rule_for_general_tag_contains(self, right_hand_term="test", operator_name="contains"):
        ...
        group = Factory.group()
        left_side = LeftSide.objects_by_account(self.request).get(display_name="have a General tag that")
        icontains = Operator.objects_by_account(self.request).get(display_name=operator_name)
        ...
        new_group_rule = GroupRule.objects_by_account(self.request).get(pk=group_rule.pk)
    ...
    def test_create_new_group_rule_for_last_volunteer_shift_is_on(self, right_hand_term="3/24/2010", operator_name="is on"):
        ...
        # create a new group rule
        left_side = LeftSide.objects_by_account(self.request).get(display_name="last volunteer shift")
        op = Operator.objects_by_account(self.request).get(display_name=operator_name)
    ...
    def test_queryset_for_new_group_rule_for_general_tag_contains(self):
        ...
        self.assertEqual(group_rule.queryset_filter_string, "filter(taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='General',name__icontains='test'))")

        # get the queryset, make sure it matches a hand-created one.
        qs = group_rule.queryset
    
        hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[0].pk) | Q(pk=ppl[2].pk) )
        ...
        hand_qs2 = Person.objects_by_account(self.request).filter(Q(pk=ppl[4].pk) )
    ...
    def test_queryset_for_new_group_rule_for_custom_tag_is_exactly(self):
        ...
        # assert the queryset string is right
        self.assertEqual(group_rule.queryset_filter_string, "filter(taggeditem__tag__in=Tag.objects_by_account(self.request).filter(tagset__name='new test tagset',name__iexact='test'))")

        # get the queryset, make sure it matches a hand-created one.
        qs = group_rule.queryset
    
        self.assertEqualQuerySets(qs,Person.objects_by_account(self.request).none())
        ...
        opposite_hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[0].pk) | Q(pk=ppl[2].pk) )
    ...
    def test_queryset_for_new_group_rule_for_last_volunteer_shift_is_on(self):
        ...
        hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[2].pk) | Q(pk=ppl[4].pk) )
        self.assertEqualQuerySets(qs,hand_qs)
    ...
    def test_queryset_for_new_group_rule_for_last_donation_is_after(self):
        ...
        hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[1].pk))
        self.assertEqualQuerySets(qs,hand_qs)

        # check the opposite
        opposite_group_rule = self.test_create_new_group_rule_for_last_donation_is_after(operator_name="is before")
        oppostite_hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[4].pk))
    ...
    def test_queryset_for_new_group_rule_for_volunteer_status_is_active(self):
        ...
        # get the queryset, make sure it matches a hand-created one.
        qs = group_rule.queryset
        hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[0].pk) | Q(pk=ppl[2].pk) | Q(pk=ppl[4].pk))

        self.assertEqualQuerySets(qs,hand_qs)

        # check the opposite
        opposite_group_rule = self.test_create_new_group_rule_for_volunteer_status_is_active(operator_name="is not")
        oppostite_hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[1].pk) | Q(pk=ppl[3].pk))
        self.assertEqualQuerySets(opposite_group_rule.queryset,oppostite_hand_qs)
    ...
    def test_queryset_for_new_group_rule_for_volunteer_status_is_inactive(self):
        ...
        # get the queryset, make sure it matches a hand-created one.
        qs = group_rule.queryset
        hand_qs = Person.objects_by_account(self.request).filter( Q(pk=ppl[2].pk) )

        self.assertEqualQuerySets(qs,hand_qs)

        # check the opposite
        opposite_group_rule = self.test_create_new_group_rule_for_volunteer_status_is_inactive(operator_name="is not")
        oppostite_hand_qs = Person.objects_by_account(self.request).filter(Q(pk=ppl[0].pk) | Q(pk=ppl[1].pk) | Q(pk=ppl[3].pk) | Q(pk=ppl[4].pk) )
        self.assertEqualQuerySets(opposite_group_rule.queryset,oppostite_hand_qs)
