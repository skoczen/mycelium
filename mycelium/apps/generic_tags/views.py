...
def save_tags_and_tagsets(request):
    ...
    for ts in data["tag_sets"]:
        ...
        if ts["db_pk"]:
            try:
                tagset = TagSet.objects.get(pk=ts["db_pk"], using="default")
                tagset.name = ts["name"]
            except TagSet.DoesNotExist: 
                pass
        else:
            tagset, created = TagSet.objects.get_or_create(account=request.account, name=ts["name"], using="default")
        ...
        if tagset:
            ...
            if ts["is_deleted"]:
                ...
                for t in ts["tags"]:
                    ...
                    if t["db_pk"]:
                        try:
                            tag = Tag.objects.get(pk=t["db_pk"], using="default")
                            tag.name = t["name"]
                            tag.tagset = tagset
                        except Tag.DoesNotExist:
                            pass
                    else:
                        tag, created = Tag.objects.get_or_create(account=request.account, name=t["name"], tagset=tagset, using="default")
